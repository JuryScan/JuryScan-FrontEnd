...
const MOCK_DATA = {
    ...
    issues: [
        { id: 1, severity: "warning", title: "Vínculo Extemporâneo Detectado", description: "O vínculo 'EMPRESA MODELO A' (Seq 01) apresenta o indicador PEXT. Este período pode não contar para aposentadoria sem prova documental.", recommendation: "Solicitar Carteira de Trabalho (CTPS) ou Ficha de Registro." },
        { id: 2, severity: "info", title: "Vínculo em Aberto", description: "O vínculo 'COMERCIO FALHO LTDA' (Seq 02) iniciou em 01/03/2020 e não possui data fim registrada.", recommendation: "Verificar se o contrato está ativo ou se é necessário acerto de vínculo com a rescisão." },
        { id: 3, severity: "critical", title: "Contribuição Abaixo do Mínimo", description: "No vínculo 'CONSULTORIA X' (Seq 03), as competências 02/2023 e 04/2023 estão abaixo do salário mínimo (Indicador PREC-MENOR-MIN).", recommendation: "Estes meses não contam para carência/tempo. Necessário complementação via DAS." }
    ]
}
...
const slugify = (str) => str.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, "-")
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  <div class="top">
    <div class="brand">JuryScan<small>Auditoria Previdenciária</small></div>
  ...
  <footer>Gerado pelo JuryScan • ${new Date().toLocaleString("pt-BR")}</footer>
</div></body></html>`
...
const buildPDF = (result) => {
    ...
    const usable = W - m * 2
    let y = 20
    const addPage = (needed = 20) => { if (y + needed > H - 18) { doc.addPage(); y = 20 } }
    const txt = (text, size, style, color) => { doc.setFontSize(size); doc.setFont("helvetica", style || "normal"); if (color) doc.setTextColor(color[0], color[1], color[2]) }

    doc.setFillColor(30, 58, 138); doc.rect(0, 0, W, 28, "F")
    txt("", 16, "bold", [255, 255, 255]); doc.text("JuryScan", m, 13)
    txt("", 10, "normal"); doc.text("Auditoria Previdenciária — Relatório de Análise de CNIS", m, 21)
    y = 38
    ...
    doc.text("CLIENTE", m + 6, y + 7); doc.text("PROCESSADO EM", m + third + 6, y + 7); doc.text("PENDÊNCIAS", m + third * 2 + 6, y + 7)
    ...
    txt("", 9, "bold", [71, 85, 105]); doc.text("PROBLEMAS IDENTIFICADOS", m, y); y += 10

    result.issues.forEach((issue, i) => {
        const s = SEVERITY_PDF[issue.severity] || SEVERITY_PDF.info
        addPage(60); const startY = y
        txt("", 12, "bold", [15, 23, 42])
        const titleLines = doc.splitTextToSize(`${i + 1}. ${issue.title}`, usable - 42)
        addPage(titleLines.length * 6 + 4); doc.text(titleLines, m + 6, y)
        txt("", 9, "bold", s.text); doc.text(SEVERITY_LABEL[issue.severity].toUpperCase(), W - m - 2, y, { align: "right" })
        y += titleLines.length * 5.5 + 8
        txt("", 8, "bold", [100, 116, 139]); doc.text("PARA O CLIENTE", m + 6, y); y += 5
        txt("", 10, "normal", [51, 65, 85])
        const descLines = doc.splitTextToSize(issue.description, usable - 12)
        addPage(descLines.length * 5 + 12); doc.text(descLines, m + 6, y); y += descLines.length * 4.6 + 8
        doc.setDrawColor(226, 232, 240); doc.line(m + 6, y - 2, W - m, y - 2)
        txt("", 8, "bold", [100, 116, 139]); doc.text("RECOMENDAÇÃO TÉCNICA", m + 6, y + 4); y += 9
        txt("", 10, "normal", [51, 65, 85])
        const recLines = doc.splitTextToSize(issue.recommendation, usable - 12)
        addPage(recLines.length * 5 + 16); doc.text(recLines, m + 6, y); y += recLines.length * 4.6 + 16
        const cardH = y - startY - 8
        doc.setFillColor(s.bar[0], s.bar[1], s.bar[2]); doc.rect(m, startY - 2, 3, cardH, "F")
        doc.setFillColor(248, 250, 252); doc.setDrawColor(226, 232, 240)
    ...
    for (let p = 1; p <= total; p++) {
        ...
        doc.text(`Página ${p} de ${total}  •  JuryScan  •  Gerado em ${new Date().toLocaleDateString("pt-BR")}`, W / 2, H - 5, { align: "center" })
    }
    return doc
}
...
export default function AuditorPage() {
    ...
    const exportPDF = () => {
        if (!result) return
        try { buildPDF(result).save(`relatorio-cnis-${slugify(result.summary.clientName)}.pdf`); showToast("PDF gerado!") }
            ...
            <main className="flex-grow py-8 px-4">
                <div className="max-w-7xl mx-auto">
                    <div className="grid lg:grid-cols-2 gap-6 items-start">
                        ...
                        <AnalysisResult result={result} loading={isLoading} onExportPDF={exportPDF} onExportHTML={exportHTML} />
                    </div>
                </div>
            </main>
            <Footer />
