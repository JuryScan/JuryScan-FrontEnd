...
import { LayoutDashboard, Users, CreditCard, Settings, LogOut, History } from 'lucide-react'
import {
  ...
  return (
    <div className="flex min-h-screen w-full">
              ...
              return (
                <SidebarMenuItem key={item.href}>
                  <SidebarMenuButton asChild isActive={isActive} tooltip={item.label}>
                    <Link href={item.href} onClick={() => setSelectedRoute(item.href)} className={`flex w-full items-center gap-3 ${isActive ? 'text-[#633B48]' : ''}`}>
      ...
      <div className="flex flex-1 flex-col min-w-0">
        <Topbar />
        <main className="flex-1 px-4 py-6 sm:px-6 lg:px-8 bg-gray-50/50 overflow-auto">
          {children}
        </main>
      </div>
    </div>
